...
if response.status_code == 200:
    ...
    if file_format == 'csv':
        fieldnames = ['key', 'component', 'line', 'message', 'severity', 'status']
        ...
        with open('issues.html', 'w') as html_file:
            html_file.write("<html><head><style>table, th, td { border: 1px solid black; border-collapse: collapse; "
                            "}</style></head><body><table><tr><th>Key</th><th>Component</th><th>Line</th><th>Message"
                            "</th><th>Severity</th><th>Status</th></tr>")
            for issue in issues:
                html_file.write("<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                    issue['key'], issue['component'], issue['line'], issue['message'], issue['severity'],
                    issue['status']
                ))
        ...
        with open('issues.md', 'w') as md_file:
            ...
            md_file.write("| Key | Component | Line | Message | Severity | Status |\n")
            md_file.write("| --- | --- | --- | --- | --- | --- |\n")
            for issue in issues:
                md_file.write("| {} | {} | {} | {} | {} | {} |\n".format(
                    issue['key'], issue['component'], issue['line'], issue['message'], issue['severity'],
