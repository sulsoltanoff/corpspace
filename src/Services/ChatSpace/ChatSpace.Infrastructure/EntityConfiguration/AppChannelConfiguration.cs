...
public class AppChannelConfiguration : IEntityTypeConfiguration<AppChannel>
{
    public void Configure(EntityTypeBuilder<AppChannel> builder)
    {
      ...
      builder.Property(appChannel => appChannel.DisplayName)
        ...
        builder.HasOne<AppChannelType>(channel => channel.AppChannelType)
            .WithMany()
            .HasForeignKey(channel => channel.AppChannelTypeId)
            .IsRequired()
            .OnDelete(DeleteBehavior.Restrict);
        
        builder.HasOne<AppTeam>(channel => channel.Team)
            .WithMany()
            .HasForeignKey(channel => channel.TeamId)
            .IsRequired(false)
            .OnDelete(DeleteBehavior.Restrict);
    }
}